...
class Check():
    ...
    @torch.no_grad()
    def _render_video(self,scene:Gaussian_Scene,save_dir='./',colorize=False,postfix=''):
        ...
        imageio.mimwrite(f'{save_dir}video_rgb_' + str(postfix) + '.mp4',rgbs,fps=20)
        imageio.mimwrite(f'{save_dir}video_dpt_' + str(postfix) + '.mp4',dpts,fps=20)
