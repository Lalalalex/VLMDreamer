...
@torch.no_grad()
@torch.inference_mode()
def process_diffusion(positive_cond, positive_cond_2, negative_cond, steps, switch, width, height, image_seed, callback, sampler_name, scheduler_name, latent=None, denoise=1.0, tiled=False, cfg_scale=7.0, refiner_swap_method='joint', disable_preview=False):
    target_unet, target_vae, target_refiner_unet, target_refiner_vae, target_clip \
        = final_unet, final_vae, final_refiner_unet, final_refiner_vae, final_clip
    print(final_refiner_unet)
    assert refiner_swap_method in ['joint', 'separate', 'vae']
    ...
    if final_refiner_vae is not None and final_refiner_unet is not None:
        ...
        if denoise > 0.9:
            ...
            if denoise > (float(steps - switch) / float(steps)) ** 0.834:  # karras 0.834
                ...
                positive_cond_2  = clip_separate(positive_cond_2, target_model=final_refiner_unet.model, target_clip=final_clip)
    ...
    if refiner_swap_method == 'separate':
        ...
        sampled_latent = core.ksampler(
            ...
            positive_2=clip_separate(positive_cond_2, target_model=target_model.model, target_clip=target_clip),
