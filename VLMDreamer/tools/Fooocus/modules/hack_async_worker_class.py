...
class Hack_worker():
    ...
    def handler(self, async_task):
                ...
                if isinstance(inpaint_image, np.ndarray) and isinstance(inpaint_mask, np.ndarray) \
                    ...
                    if inpaint_additional_prompt != '':
                        if prompt == '' or prompt_2 == '':
                            ...
                            prompt_2 = inpaint_additional_prompt
                        elif prompt == '':
                            prompt = inpaint_additional_prompt
                        elif prompt_2 == '':
                            prompt_2 =inpaint_additional_prompt
        ...
        if not skip_prompt_processing:
            ...
            prompts_2 = remove_empty_str([safe_str(p) for p in prompt_2.splitlines()], default='')
            negative_prompts = remove_empty_str([safe_str(p) for p in negative_prompt.splitlines()], default='')

            prompt = prompts[0]
            prompt_2 = prompts_2[0]
            ...
            for i in range(image_number):
                ...
                task_prompt_2 = apply_wildcards(prompt_2, task_rng, i, read_wildcards_in_order)
                task_prompt = apply_arrays(task_prompt, i)
                task_prompt_2 = apply_arrays(task_prompt_2, i)
                ...
                if use_style:
                    for i, s in enumerate(task_styles):
                        ...
                        p_2 ,n = apply_style(s, positive=task_prompt_2)
                        positive_basic_workloads = positive_basic_workloads + p
                        positive_2_basic_workloads = positive_2_basic_workloads + p_2
                        negative_basic_workloads = negative_basic_workloads + n
                else:
                    positive_basic_workloads.append(task_prompt)
                    positive_2_basic_workloads.append(task_prompt_2)

                negative_basic_workloads.append(task_negative_prompt)  # Always use independent workload for negative.

                positive_basic_workloads = positive_basic_workloads + task_extra_positive_prompts
                positive_2_basic_workloads = positive_2_basic_workloads + task_extra_positive_prompts
                negative_basic_workloads = negative_basic_workloads + task_extra_negative_prompts

                positive_basic_workloads = remove_empty_str(positive_basic_workloads, default=task_prompt)
                positive_2_basic_workloads = remove_empty_str(positive_2_basic_workloads, default=task_prompt_2)
                negative_basic_workloads = remove_empty_str(negative_basic_workloads, default=task_negative_prompt)

                tasks.append(dict(
                    task_seed=task_seed,
                    task_prompt=task_prompt,
                    task_prompt_2=task_prompt_2,
                    task_negative_prompt=task_negative_prompt,
                    positive=positive_basic_workloads,
                    positive_2=positive_2_basic_workloads,
                    ...
                    log_positive_prompt_2='\n'.join([task_prompt_2] + task_extra_positive_prompts),
            ...
            if use_expansion:
                for i, t in enumerate(tasks):
                    ...
                    t['positive_2'] = copy.deepcopy(t['positive_2']) + [expansion]  # Deep copy.

            for i, t in enumerate(tasks):
                self.progressbar(async_task, 5, f'Encoding positive #{i + 1} ...')
                t['c'] = pipeline.clip_encode(texts=t['positive'], pool_top_k=t['positive_top_k'])
                t['c_2'] = pipeline.clip_encode(texts=t['positive_2'], pool_top_k=t['positive_top_k'])
        ...
        for current_task_id, task in enumerate(tasks):
            ...
            try:
                ...
                #print(positive_cond)
                positive_cond_2, negative_cond = task['c_2'], task['uc']
                ...
                if 'cn' in goals:
                    for cn_flag, cn_path in [
                        ...
                        for cn_img, cn_stop, cn_weight in cn_tasks[cn_flag]:
                            ...
                            positive_cond_2, negative_cond = core.apply_controlnet(
                                positive_cond_2, negative_cond,
                                pipeline.loaded_ControlNets[cn_path], cn_img, cn_weight, 0, cn_stop)

                imgs = pipeline.process_diffusion(
                    positive_cond=positive_cond,
                    positive_cond_2=positive_cond_2,
                ...
                del task['c'],task['c_2'] , task['uc'], positive_cond, positive_cond_2 , negative_cond  # Save memory
